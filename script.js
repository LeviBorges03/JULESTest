...
function criarAlvo() {
    ...
    const alvoState = {
        ...
        x: x,
        y: y,
        lineSegment: { x1: x - 25, y1: y + 30.5, x2: x + 25, y2: y - 29.5 }
    };
    ...
    TweenMax.to(novoAlvoG, 0.5, { scale: 1, ease: Back.easeOut, delay: 0.3 });
...
function loose() {
	...
	TweenMax.to([newArrow], 0.5, { force3D: true, bezier: { type: "cubic", values: path, autoRotate: ["x", "y", "rotation"] }, onUpdate: hitTest, onUpdateParams: ["{self}"], onComplete: onMiss, onCompleteParams: ["{self}"], ease: Linear.easeNone });
...
function hitTest(tween) {
    ...
    for (let i = gameState.alvos.length - 1; i >= 0; i--) {
        ...
        if (intersection.segment1 && intersection.segment2) {
            ...
            TweenMax.to(arrow, 0.5, { opacity: 0, delay: 4, onComplete: () => arrow.remove() });
            ...
            if (distance < 7) {
                ...
                if (gameState.alvos.length < CONFIG.MAX_ALVOS && gameState.alvos.length > 0) {
                    criarAlvo();
                } else if (gameState.alvos.length === 0) {
                    criarAlvo();
                }
            }
