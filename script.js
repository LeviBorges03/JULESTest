...
function criarAlvo() {
    ...
    EFEITOS.particulas(x, y, "#FFF", 20); // Emite partículas brancas no local do alvo
    ...
    for (let i = gameState.alvos.length - 1; i >= 0; i--) {
        ...
        if (intersection.segment1 && intersection.segment2) {
            ...
            if (distance < 7) {
                ...
                if (gameState.alvos.length < CONFIG.MAX_ALVOS) {
                    criarAlvo();
                }
            }
...
function onMiss(tween) {
    ...
    EFEITOS.particulas(transform.x, transform.y, "#888888", 10);
	showMessage(".miss");
    morrer();
}
