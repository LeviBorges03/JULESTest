...
let botoesEfeitosCriados = false; // Variável de controle
...
function criarAlvo() {
    ...
    // Posição aleatória, evitando a área do arco
    const x = Math.random() * 400 + 500;
    ...
    // Adiciona o novo alvo ao SVG principal
    ...
    const alvoState = {
        ...
        x: x,
        y: y,
        // O segmento de linha para detecção de colisão precisa ser relativo à nova posição
        lineSegment: {
            x1: x - 25, y1: y + 30.5,
            x2: x + 25, y2: y - 29.5
        }
    ...
    // Animação de surgimento com partículas
    EFEITOS.particulas.action(x, y, "#FFF", 20); // Emite partículas brancas no local do alvo

    // Animação de entrada
    TweenMax.set(novoAlvoG, { scale: 0, transformOrigin: "center center" });
    TweenMax.to(novoAlvoG, 0.5, { scale: 1, ease: Back.easeOut, delay: 0.3 }); // Atraso para as partículas aparecerem primeiro
    TweenMax.to(novoAlvoG, 0.1, { attr: { transform: `translate(${x - 900}, ${y - 249.5})` } });
}
...
function iniciarJogo() {
    ...
    gameState.particulasAcumuladas = 0; // Reseta as partículas

    // Remove alvos antigos e reseta o array
    ...
    criarAlvo(); // Cria o primeiro alvo
}
...
function hitTest(tween) {
	...
	var arrowSegment = {
		x1: transform.x,
		y1: transform.y,
		x2: (Math.cos(radians) * 15) + transform.x,
		y2: (Math.sin(radians) * 15) + transform.y
	}

    // Itera sobre todos os alvos para verificar a colisão
    for (let i = gameState.alvos.length - 1; i >= 0; i--) {
        const alvo = gameState.alvos[i];
        var intersection = getIntersection(arrowSegment, alvo.lineSegment);

        if (intersection.segment1 && intersection.segment2) {
            // Desativa a atualização da posição da flecha para que ela "grude" no alvo
            tween.pause();

            // Fade out e remoção da flecha após um tempo
            const arrow = tween.target[0];
            TweenMax.to(arrow, 0.5, { opacity: 0, delay: 4, onComplete: () => {
                arrow.remove();
            }});

            var dx = intersection.x - alvo.x;
            var dy = intersection.y - alvo.y;
            ...
            if (distance < 7) {
                selector = ".bullseye";
                gameState.score++;
                gameState.particulasAcumuladas += 2;
                atualizarPlacar();
                ...
                // Adiciona um novo alvo se não atingiu o limite
                if (gameState.alvos.length < CONFIG.MAX_ALVOS && gameState.alvos.length > 0) { // Garante que o jogo não termine
                    criarAlvo();
                } else if (gameState.alvos.length === 0) { // Se não houver mais alvos, cria um novo
                    criarAlvo();
                }
            }

            // Aciona os efeitos visuais selecionados
            personagem.efeitos.forEach(efeito => {
                if (EFEITOS[efeito] && EFEITOS[efeito].action) {
                    const quantidade = efeito === 'particulas' ? gameState.particulasAcumuladas : 15;
                    EFEITOS[efeito].action(intersection.x, intersection.y, personagem.cor, quantidade);
                }
            });

            showMessage(selector);
            return; // Encerra a função após o primeiro acerto
        }
    }
}

function onMiss(tween) {
    // Damn!
    const arrow = tween.target[0];
    const transform = arrow._gsTransform;
    // Aciona um efeito visual de "erro"
...
function setup() {
    ...
    function carregarPersonagem(slot) {
        ...
        if (salvo) {
            ...
            // Garante que 'efeitos' seja sempre um array
            ...
            desenharPreview();
            popularEfeitos(); // Garante que os botões de efeito sejam atualizados
        }
    }
    ...
    function popularEfeitos() {
        // Apenas cria os botões na primeira vez
        if (!botoesEfeitosCriados) {
            listaEfeitosContainer.innerHTML = "";

            const efeitosArray = Object.keys(EFEITOS)
                .filter(key => !key.startsWith('_'))
                .map(key => ({ key: key, ...EFEITOS[key] }));
            ...
            efeitosArray.forEach(efeito => {
                ...
                btn.textContent = efeito.simbolo; // Usa o símbolo em vez da chave
                btn.title = key; // Adiciona o nome do efeito como tooltip
                btn.classList.add("btn-efeito");
                btn.dataset.effectKey = key; // Guarda a chave do efeito no botão
                ...
                btn.onclick = () => {
                    ...
                    atualizarBotoesEfeitos(); // Apenas atualiza o estado visual
                    desenharPreview();
                };
        ...
        atualizarBotoesEfeitos(); // Atualiza o estado visual em todas as chamadas
    }
    ...
    function atualizarBotoesEfeitos() {
        ...
        botoes.forEach(btn => {
            ...
            if (personagem.efeitos.includes(key)) {
    ...
    function desenharPreview() {
        ...
        // Simula os efeitos selecionados no preview
        personagem.efeitos.forEach(key => {
            if (key.startsWith('particulas') || key === 'particulas') { // Verifica se é o efeito de partículas
                 for (let i = 0; i < 5; i++) {
            ...
            // Adicionar simulações para outros tipos de efeitos aqui se desejar
        });
    ...
    // popularEfeitos é chamado na primeira vez que o menu abre
...
// Configuração inicial do evento de clique, espelhando a lógica original do Archer
